...
### setBlob
response_setBlob_test=`curl -d "{ \"auth_token\" : "$auth_token", \"guid\" : \"\", \"blob\" : \"\" }" http://${INSTANCE}/service/setBlob`;
echo "SetBlob test - $response_setBlob_test"
errno_incorrect_json="9";
checkResponseErrno "$response_setBlob_test" $errno_incorrect_json "Fail at setBlob test"

### getBlob
response_getBlob_test=`curl -d "{ \"auth_token\" : "$auth_token", \"guid\" : \"\" }" http://${INSTANCE}/service/getBlob`;
echo "GetBlob test - $response_getBlob_test"
errno_incorrect_json="9";
checkResponseErrno "$response_getBlob_test" $errno_incorrect_json "Fail at getBlob test"
